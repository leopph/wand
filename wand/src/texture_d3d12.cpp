...
namespace wand {
TextureD3D12::TextureD3D12(Desc const& desc, DeviceD3D12* const device,
                           Microsoft::WRL::ComPtr<D3D12MA::Allocation> alloc,
                           std::uint32_t const srv_idx,
                           std::uint32_t const uav_idx,
                           std::uint32_t const rtv_idx,
                           std::uint32_t const dsv_idx,
                           bool const is_swap_chain_buffer) : Texture{desc},
  device_{device}, alloc_{std::move(alloc)}, srv_idx_{srv_idx},
  uav_idx_{uav_idx}, rtv_idx_{rtv_idx}, dsv_idx_{dsv_idx},
  is_swap_chain_buffer_{is_swap_chain_buffer} {
}

TextureD3D12::~TextureD3D12() {
  if (!is_swap_chain_buffer_) {
